...
// are no assertions. Build with `cmake --build . --target smooth-movement-bench`, or
// `c++ -std=c++17 -O2 bench_visual_animation_manager.cpp`, and run it before and after a change.
...
struct viewport_arraysst
{
	...
	// The game's redraw: current becomes previous, then current is drawn again.
	void advance()
		{
...
void draw(viewport_arraysst &arrays,const std::vector<creaturest> &creatures)
{
	...
	for(const creaturest &creature:creatures)
		{
		arrays.current[center][size_t(creature.x)*dim_y+size_t(creature.y)]=creature.texpos;
		if(creature.x+1<dim_x&&creature.texpos%3==0)
			arrays.current[right][size_t(creature.x+1)*dim_y+size_t(creature.y)]=creature.texpos+1;
		}
}
...
void run(int32_t creature_count)
{
	...
	for(int frame=0;frame<frames;++frame)
		...
		manager.synchronize_viewport(arrays.input());
		manager.end_frame();
		const uint64_t t1=now_us();
		// What collect_proxies does: one lookup per drawn tile in every layer, y outer like the plugin.
		for(size_t layer=0;layer<layer_count;++layer)
			{
			const std::vector<int32_t> &current=arrays.current[layer];
			for(int32_t y=0;y<dim_y;++y)
				for(int32_t x=0;x<dim_x;++x)
					...
					const visual_movement_renderst movement=manager.get_movement(
						&arrays,static_cast<viewport_visual_layer>(layer),x,y);
					if(movement.active)++active;
					}
			}
		const uint64_t t2=now_us();
		// What the resting mirrored pass does: one facing lookup per tile, x outer like the plugin.
		for(int32_t x=0;x<dim_x;++x)
			for(int32_t y=0;y<dim_y;++y)
				if(manager.get_facing(&arrays,x,y)!=native_sprite_facing)++mirrored;
	...
	std::printf("%4d creatures: sync %6.1f us  movement lookups %6.1f us  facing lookups %6.1f us  per frame (%.1f active, %.1f mirrored lookups/frame)\n",
		creature_count,double(sync_us)/frames,double(lookup_us)/frames,double(facing_us)/frames,double(active)/frames,double(mirrored)/frames);
}
