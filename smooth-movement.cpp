...
// Frame recorder for `smooth-movement stats record`: writes what the render hook reads before
...
struct frame_recorderst
{
	...
	// Raw bytes of every buffer as of the last captured frame, per slot and buffer.
	...
	// Both called by the render hook; capture before the frame, finish after it.
	void capture(df::renderer_2d_base *renderer,uint32_t now_ms);
	// Words of the active viewports' buffers that differ from the captured copy: the game
	...
	std::vector<std::string> &parameters)
{
	...
	if(parameters[0]=="stats")
		...
		if(parameters.size()>=3&&parameters.size()<=4&&parameters[1]=="record")
			{
			if(parameters[2]=="stop")
				{
			...
			if(parameters.size()==4)
				{
				try{frames=uint32_t(std::stoul(parameters[3]));}
				catch(...){return CR_WRONG_USAGE;}
				if(frames==0)return CR_WRONG_USAGE;
				}
			if(!frame_recorder.start(parameters[2],frames))
				{
...
plugin_init(color_ostream &,std::vector<PluginCommand> &commands)
{
	commands.emplace_back(
		...
		"frame recording for the offline harness: stats record <file> [frames] | stats record stop.",
		status_command);
	return CR_OK;
}
