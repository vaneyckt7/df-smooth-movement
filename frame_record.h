...
// (writer, `smooth-movement stats record`) and the harness (reader, `bench replay`).
//
...
//         i32 zoom origin_x origin_y dimx dimy
//     u8 viewports; per viewport: u8 slot (0..7 lower, 8 main), i32 dim_x dim_y clipx0 clipx1
//         clipy0 clipy1 screen_x screen_y, then every buffer in `for_each_buffer` order as
//         u8 present and, when present, a run-coded array
//     u32 units; per unit: i32 x y z, i32 hauled item texpos, u8 texture already cached
//     'E' u32 engine repaints this frame, u8 painted, u32 buffer words that had changed under
//         the hook by the time it returned (a frame the replay cannot reproduce exactly)
// Run coding of an array of N words: varint (length<<2 | op) where op 0 = zeros, 1 = same
// words as this buffer had in the previous frame, 2 = literal words follow.
#pragma once
...
// Every per-tile buffer of a viewport, current then old, in declaration order.
...
struct writerst
{
	...
	// Run-codes `count` words of `current` against `previous` (the same buffer one frame
...
struct readerst
{
	...
	// Decodes in place: `words` must still hold the previous frame's values of this buffer.
	template<typename T>
