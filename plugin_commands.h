...
void print_setting(Output &out,const plugin_statest &state,const std::string &word)
{
	...
	if(word=="hop")out.print("walk hop: amount {:.2f}\n",movement_setting(hop,"amount"));
	if(word=="hopmult")
...
void print_settings(Output &out,const plugin_statest &state,const command_hostst &host)
{
	...
	for(const char *word:{"flip","interpolation","timestep","hauled","hop","hopmult","hops"})
	...
	const command_hostst &host)
{
	...
	if(parameters.size()!=2)return command_outcomest::wrong_usage;
	// The amount, in tiles. It only sets the height: the hop is picked with
	// `interpolation hop`, so zero is rejected with the rest (by the movement).
