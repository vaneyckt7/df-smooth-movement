...
- Added: `interpolation <name>` picks how a step is spread over frames from three movements behind one interface (`movement.h`): `smoothstep` (the default), `linear` and `hop`. A movement is given the moment of the step and the step as a tile delta and answers with the sprite's offset from the old tile, in tiles; whatever it does between the tiles is its own. It also declares its overshoot, how far beyond the straight path it can take the sprite in each direction, which is the box of extra tiles the plugin erases and repaints around a moving sprite, and it owns its settings as named numbers: the walk hop's `amount`, `horizontal`, `diagonal`, `vertical` and `hops`, which the console words `hop`, `hopmult` and `hops` set by name. A creature whose overshoot tiles are off the screen or burning glides on the line at the movement's own pace for that step, where the release only dropped the hop. A movement may reach the tile before the step's time is up and hold there, and the plugin stops drawing the step once it has, since the game draws the creature on its tile; none of the three does yet. `interpolation <name>` is the one way to pick a movement: `linear on|off` and `hop on|off` are gone, `all on|off` switches `flip` and `hauled` and leaves the interpolation alone, and the status printout's `linear movement` line is gone with its `walk hop` line saying the amount only. Frame recordings store the movement by name with its settings by name (format version 7); older recordings read as before, the linear switch (and, in version 5, the hop switch) naming one of the three and the fixed hop fields becoming the `hop` movement's settings, and a version 5 frame with both switches on fails to read. A disabled and re-enabled plugin keeps the interpolation, `hop` included, where it kept `linear` alone. `recinfo.py` prints the name and the settings. A sprite's pixel position is now the offset in tiles times the tile size, and on a step retargeted in flight or a hopping sprite that can land one thousandth of a pixel from where the release put it.
- Changed: every interpolation follows the creature's cadence, where only `linear` did. The first step of a walk takes the `timestep`; a step that continues an earlier one takes the time since that step started, up to 500 ms or the timestep when that is longer, and a finished step stays usable as the predecessor of the next for that long. The cadence is no longer floored at the timestep, so a creature that steps faster than the timestep no longer falls behind its tile, a little more with every step: it glides each tile in the time between its steps. With `smoothstep` and `hop`, a creature that steps faster than the timestep now glides faster, and one that steps slower, up to 500 ms, glides without a pause between steps where it used to glide for the timestep and wait. In the harness replays 323 of the 600 frames of `fortress-600` and 22 of the 183 of `fortress-camera-183` draw differently.
- Changed: the plugin's settings are printed by `smooth-movement status`; the bare `smooth-movement` prints the usage, like any other word the console does not know.
...
- Add `smooth-movement timestep <ms>`: how long a one-tile glide takes, 20 to 2000 ms,
  ...
  (the floor is gone since; see the cadence entry above).
