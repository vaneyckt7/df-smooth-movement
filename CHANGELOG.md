...
- Added: `interpolation <name>` picks how a step is spread over frames from three movements behind one interface (`movement.h`): `smoothstep` (the default), `linear` and `bob`. A movement is given the moment of the step and the step as a tile delta and answers with the sprite's offset from the old tile, in tiles; whatever it does between the tiles is its own. It also declares its reach, how far above and below the line it can take the sprite, which is the box of extra rows the plugin erases and repaints around a moving sprite (one at most, so settings reaching a whole tile are refused), and it owns its settings as named numbers: the walk bob's `amount`, `horizontal`, `diagonal`, `vertical` and `hops`, which the console words `bob`, `bobmult` and `hops` set by name. A creature whose reach rows are off the screen or burning glides with `smoothstep` for that step, where the release only dropped the bob. `interpolation <name>` is the one way to pick a movement: `linear on|off` and `bob on|off` are gone, `all on|off` switches `flip` and `hauled` and leaves the interpolation alone, and the status printout's `linear movement` line is gone with its `walk bob` line saying the amount only. Frame recordings store the movement by name with its settings by name (format version 7); older recordings read as before, the linear switch (and, in version 5, the bob switch) naming one of the three and the fixed bob fields becoming the `bob` movement's settings, and a version 5 frame with both switches on fails to read. A disabled and re-enabled plugin keeps the interpolation, `bob` included, where it kept `linear` alone. `recinfo.py` prints the name and the settings. What the plugin draws is unchanged to within float rounding: a sprite's pixel position is now the offset in tiles times the tile size, and on a step retargeted in flight or a bobbing sprite that can land one thousandth of a pixel from where the release put it.
