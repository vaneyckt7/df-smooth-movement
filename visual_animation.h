...
class visual_animation_managerst
{
	...
	float movement_travelled_pct(const movement_recordst &movement) const
		{
		return current_movement->travelled_pct(step_progress(
	...
	visual_movement_renderst movement_render(
		...
		const float along_pct=movement_travelled_pct(movement);
		return {true,source_x,source_y,offset.x_tiles,offset.y_tiles,
			(float(movement.target_x)-movement.source_x)*along_pct,
			(float(movement.target_y)-movement.source_y)*along_pct,inherited,movement.id};
		}
						...
						for(int32_t y=0;y<input.dim_y;++y)
							...
							if(predecessor!=nullptr)
								...
								if(movement_active(*predecessor))
									{
									const float progress_pct=movement_travelled_pct(*predecessor);
									visual_source_x=predecessor->source_x+
										(predecessor->target_x-predecessor->source_x)*progress_pct;
									visual_source_y=predecessor->source_y+
										(predecessor->target_y-predecessor->source_y)*progress_pct;
									}
				...
				for(const movement_recordst &movement:state.movements)
					if(movement.id==movement_id&&movement_active(movement))
						...
						return {true,movement.id,
							(movement.target_x-movement.source_x)*remaining_pct,
							(movement.target_y-movement.source_y)*remaining_pct};
						}
				break;
				}
			return {};
...
{
	const bool same_x=target_x>-0.5f+source_x&&target_x<=0.5f+source_x;
	const bool same_y=target_y>-0.5f+source_y&&target_y<=0.5f+source_y;
