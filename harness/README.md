...
the hop off; the hop's proxy marking and coverage are covered by the sprite proxy test. They
predate format version 6 too, which stores the interpolation by name where they have a linear
switch, and version 7, which stores the movement's settings by name; their switch is off, so
they replay with the default `smoothstep` movement, which has no settings.
