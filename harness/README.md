...
at all. `stats record stop` ends a recording early. Recording costs about a millisecond per
frame, so do not take timings while it runs. The file is written relative to the game's
directory; `frame_record.h` documents the format.
...
example `git archive release/v0.5.0 | tar -x -C harness/out/src-base`.
...
embark replay with none, as of this writing). Each frame also records how many buffer words
