...
# painting, viewport collection, free camera, view context and console command tests) against
# the stub headers in stubs/, then replays the recording the codec test writes, which is
# three frames in the current format naming a different movement each, so that the replay's
# two-pass decode of a frame header is exercised without a fixture, then replays every
# recording in recordings/ and requires that every frame's draws digest to what
# expected/<name>.digest holds, and that recinfo.py reads the same number of frames.
...
# -ffp-contract=off for the same reason build.sh sets it: the tests compare floats the
# plugin computes, and a fused multiply-add rounds differently from a separate pair.
c++ -std=c++17 -O2 -ffp-contract=off ${CXXFLAGS:-} -Wall -Wextra -I"$src" \
...
run_test() {
	c++ -std=c++17 -O2 -ffp-contract=off ${CXXFLAGS:-} -Wall -Wextra -I"$here/stubs" -I"$src" \
