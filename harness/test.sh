...
# the frame recording codec test, and a record-and-replay round trip of the built-in scene.
...
# The plugin's own recorder captures the built-in scene; replaying it must give the same draws.
"$here/build.sh" "$src" selftest
"$here/out/bench-selftest" selftest "$here/out/selftest.rec" "$here/out/selftest-record.trace" >/dev/null
"$here/out/bench-selftest" replay "$here/out/selftest.rec" "$here/out/selftest-replay.trace" | tail -2
