...
// replays frames recorded in the game with `smooth-movement stats record`, and writes every
// engine repaint and SDL draw as a trace for byte-for-byte comparison between plugin versions.
//
// Build: c++ -std=c++17 -O2 -Istubs -I<plugin dir> -DPLUGIN_SOURCE='"<plugin dir>/smooth-movement.cpp"' bench.cpp
// Run:   ./bench replay <recording> [trace-file]     frames recorded in the game
//        ./bench selftest <recording> [trace-file]   record a small built-in scene, for test.sh
...
int main(int argc,char **argv)
{
	...
	if(argc<3||strcmp(argv[1],"selftest")){fprintf(stderr,"usage: bench selftest <recording> [trace] | bench replay <recording> [trace]\n");return 2;}
	...
	for(int32_t t=100;t<130;++t){df::texture_fullid id;id.texpos=t;id.r=id.g=id.b=1.0f;id.br=id.bg=id.bb=0.0f;id.flag=df::texture_fullid_flag::mask_transparent_background;renderer.tile_cache.tile_cache[id]=reinterpret_cast<void*>(intptr_t(t));}
	...
	// hook returns before capturing units), and one scroll near the end whose buffers land two
	// frames after the window moved, like the engine does.
	...
	for(int f=-warmup;f<frames;++f)
		...
		if(f==0)
			{
			std::vector<std::string> p{"stats","record",argv[2],std::to_string(frames)};
			if(status_command(out,p)!=DFHack::CR_OK){fprintf(stderr,"%s",out.captured.c_str());return 1;}
			if(trace_path!=nullptr)trace=fopen(trace_path,"w");	// warm-up draws are not in the recording
			}
		paused=f>=120&&f<150;
