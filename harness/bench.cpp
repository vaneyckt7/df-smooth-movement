...
// An array a recording marks absent reads as zero, as the game's renderer would find nothing
// to draw from it; the plugin only requires the arrays it reads itself to be present.
uint64_t tile_hash(const df::graphic_viewportst *vp,int32_t index,bool &any)
{
	uint64_t h=0xcbf29ce484222325ULL;
	const auto mix=[&](auto *array){const uint64_t v=array?uint64_t(array[index]):0;any=any||v!=0;h=(h^v)*0x100000001b3ULL;};
	mix(vp->screentexpos_background);mix(vp->screentexpos_floor_flag);
	mix(vp->screentexpos_background_two);mix(vp->screentexpos_liquid_flag);
	mix(vp->screentexpos_spatter_flag);mix(vp->screentexpos_spatter);
	mix(vp->screentexpos_ramp_flag);mix(vp->screentexpos_shadow_flag);
	mix(vp->screentexpos_building_one);mix(vp->screentexpos_item);
	mix(vp->screentexpos_vehicle);mix(vp->screentexpos_vermin);
	mix(vp->screentexpos_left_creature);mix(vp->screentexpos);
	mix(vp->screentexpos_right_creature);mix(vp->screentexpos_building_two);
	mix(vp->screentexpos_projectile);mix(vp->screentexpos_high_flow);
	mix(vp->screentexpos_top_shadow);mix(vp->screentexpos_signpost);
	mix(vp->screentexpos_upleft_creature);mix(vp->screentexpos_up_creature);
	mix(vp->screentexpos_upright_creature);mix(vp->screentexpos_designation);
...
int run_replay(const char *record_path,const char *trace_path)
{
	...
	while(decode_frame(reader,slots,graphics,frame,nullptr))
		...
		state.apply_settings(h.settings);
