...
int main(int argc,char **argv)
{
	...
	frame_record::writerst w;
	frame_record::write_file_header(w);
	frame_record::frame_headerst f;f.flip=true;f.camera=true;f.step_ms=300;f.simulation_tick=int64_t(3)<<33;f.tick_ms=123456;f.window_x=-3;f.window_z=77;f.paused=true;f.follow_unit=9;f.mouse_x=-1;f.mouse_mbut=true;f.zoom=96;f.origin_x=5;f.dimx=200;f.rest_x=-0.25;f.rest_y=1.5;
	frame_record::write_frame_header(w,f);
	...
	// back as unknown; a version past the current one is rejected, as is a step of zero
	// and a tick below -1.
	{
	frame_record::writerst w;
	...
	frame_record::write_frame_header(w,f);
	w.bytes.erase(w.bytes.begin()+8+5,w.bytes.begin()+8+17); // drop the step and tick fields
	frame_record::readerst r;r.data=w.bytes.data();r.size=w.bytes.size();
	...
	w1.bytes.erase(w1.bytes.begin()+8+9,w1.bytes.begin()+8+17); // drop the tick field
