...
int main(int argc,char **argv)
{
	...
	const std::vector<movement_settingst> bob_settings={{"amount",0.25f},{"horizontal",1.5f},
		{"diagonal",2.0f},{"vertical",3.0f},{"hops",1.0f}};
	{
	frame_record::writerst w;
	frame_record::write_file_header(w);
	...
	f.settings.movement="bob";f.settings.movement_settings=bob_settings;
	...
	if(f2.settings.movement_settings!=bob_settings)
	...
	// back as unknown, a version 4 file no walk bob settings, a version 5 file a linear
	// switch and a bob switch that name the movement and fixed walk bob fields, a version 6
	...
	// walk bob fields of versions 5 and 6, then the rest as the current writer lays it out.
	struct old_bobst{float amount,horizontal,diagonal,vertical;uint8_t hops;};
	const auto write_old_header=[](frame_record::writerst &w,uint32_t version,
		const frame_record::frame_headerst &g,bool linear,bool bob,const old_bobst &fields)
		{
	...
	const auto read_old=[&](uint32_t version,bool linear,bool bob,
		frame_record::frame_headerst &out)
		{
		frame_record::writerst w;
		w.raw("SMRC",4);w.u32(version);
		write_old_header(w,version,f,linear,bob,fields);
		frame_record::readerst r;r.data=w.bytes.data();r.size=w.bytes.size();
		out=f;
	...
	// A version 5 header's two switches name the movement, and its fixed walk bob fields
	// read back as the bob's settings when the bob is named, nothing otherwise; both
	...
	for(const uint32_t version:{5u,6u})
		for(const auto &[linear,bob,name]:{
			...
			write_old_header(w,version,g,linear,bob,fields);
			frame_record::readerst r;r.data=w.bytes.data();r.size=w.bytes.size();
			frame_record::frame_headerst out=f;
			const std::vector<movement_settingst> expected=
				std::string(name)=="bob"?fields_as_settings:std::vector<movement_settingst>{};
	...
	// A version 5 or 6 header whose fixed fields the bob would refuse is rejected when the
	// bob is named, and read when it is not, since the fields then set nothing.
	for(const uint32_t version:{5u,6u})
		{
		const old_bobst bad{0.4f,1.0f,2.4f,2.7f,3};
		for(const auto &[linear,bob,name,accepted]:{
			std::tuple<bool,bool,const char *,bool>{false,true,"bob",false},
			{true,false,"linear",true}})
			{
			frame_record::frame_headerst g=f;g.settings.movement=name;
			g.settings.movement_settings.clear();
			frame_record::writerst w;w.raw("SMRC",4);w.u32(version);
			write_old_header(w,version,g,linear,bob,bad);
			frame_record::readerst r;r.data=w.bytes.data();r.size=w.bytes.size();
			frame_record::frame_headerst out;
			const bool read=frame_record::read_file_header(r)&&
				frame_record::read_frame_header(r,out)&&r.at_end();
			if(read!=accepted)
				{printf("version %u header with bad bob fields as %s: %s\n",version,name,
	...
	// product with a multiplier passes the cap, a NaN multiplier, a multiplier past 5 on
	// each field, a setting the movement does not have, and a setting on a movement that
	// has none.
	...
	rejects("bob",{{"amount",0.0f}},"zero bob amount");
	rejects("bob",{{"hops",3.0f}},"three hops");
	rejects("bob",{{"amount",0.4f}},"bob reach past the cap");
	rejects("bob",{{"diagonal",std::nanf("")}},"NaN bob multiplier");
	rejects("bob",{{"amount",0.1f},{"horizontal",6.0f}},"horizontal multiplier past 5");
	rejects("bob",{{"amount",0.1f},{"diagonal",6.0f}},"diagonal multiplier past 5");
	rejects("bob",{{"amount",0.1f},{"vertical",6.0f}},"vertical multiplier past 5");
	rejects("bob",{{"stride",1.0f}},"a setting the bob lacks");
