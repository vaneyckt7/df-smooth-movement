...
int main()
{
	...
	// A run longer than the buffer must be rejected, not written past it.
	{
	...
	for(size_t i=50;i<60;++i)if(out[i]!=-1){puts("overlong run wrote past the buffer");++failures;break;}
	}
