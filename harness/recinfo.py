...
Usage: recinfo.py <recording> [first frame] [last frame]
    ...
    $ harness/recinfo.py harness/out/live10.rec 0 9
    version 2
...
and exist for the replay: `bench replay` (the harness in this directory) decodes them into a stub
viewport and runs the plugin's real render hook on them, then compares the repaints it makes
with the repaints recorded here. This script decodes each array's run lengths only to find
